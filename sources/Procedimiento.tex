...
\begin{itemize}
    ...
    \item Tres recipientes plásticos para colgar las masas.
    \item Cinta adhesiva.
...
\begin{itemize}
    ...
    \begin{itemize}
        ...
        \item Masa 3 ($m_3$): Varía entre $0.303$ a $0.564$ kg $\rightarrow$ Peso $\approx 2.97$ a $5.53$ N
    \end{itemize}
    \vspace{0.2cm}
...
\begin{enumerate}
    \item Se midió la masa de la viga, obteniéndose un valor aproximado de $0,770$ kg.
    ...
    \item Con las masas $m_1$ y $m_2$ ubicadas en sus respectivas posiciones, se ajustó la masa o la distancia de $m_3$, según correspondiera en cada caso, con el fin de mantener la viga en equilibrio.

    \item El procedimiento se repitió para las configuraciones del orificio A, modificando las masas y las distancias según cada caso.

    \item Posteriormente, la viga se suspendió desde el orificio B y se repitió el mismo procedimiento. En esta configuración se consideró además el torque producido por el peso de la propia viga, debido a que el punto de suspensión no coincidía con su centro de masa.

    \item Finalmente, se calcularon los torques mediante la expresión:
    \[
        \tau = r \cdot F = r \cdot m \cdot g
    \]
    y se comparó la sumatoria experimental con la condición teórica de equilibrio rotacional utilizando:
    \[
        \sum \tau = 0.
    \]
\end{enumerate}